...
from s_e.aufbau_character_matrix import analyze_text, analyze_files
...
def test_analyze_many_files(data_folder):
    ...
    result = analyze_files(files)
    assert len(result.index) > 3
    assert len(result.columns) == 10    #76
# Anzahl files + summenspalte = 10 manuell ermittelt